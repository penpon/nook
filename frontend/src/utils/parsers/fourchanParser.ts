...
export function parseFourchanThreadsMarkdown(markdown: string): ContentItem[] {
    ...
    // スレッドタイトル（### [タイトル](URL)）
      ...
      if (linkMatch) {
        ...
        for (let j = i + 1; j < lines.length; j++) {
          ...
          } else if (nextLine.startsWith('**画像数**:')) {
            imageCount = nextLine.replace('**画像数**:', '').trim();
