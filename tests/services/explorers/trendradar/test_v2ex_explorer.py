...
from nook.services.explorers.trendradar.v2ex_explorer import V2exExplorer
...
def explorer(mock_trendradar_client):
    return V2exExplorer(storage_dir="test_data")


@pytest.mark.asyncio
async def test_initialization(explorer):
    """初期化のテスト."""
    assert explorer.service_name == "trendradar-v2ex"
    assert explorer.PLATFORM_NAME == "v2ex"
    assert explorer.FEED_NAME == "v2ex"
    assert explorer.MARKDOWN_HEADER == "V2EXトレンドトピック"
...
async def test_get_summary_prompt(explorer):
    ...
    article = Article(
        feed_name="v2ex",
        title="Test V2EX Title",
        url="http://example.com",
        text="Test V2EX Description",
    ...
    assert "V2EXトレンドトピック" in prompt
    assert "Test V2EX Title" in prompt
    assert "Test V2EX Description" in prompt
    assert "トピックの概要" in prompt
    assert "議論のポイント" in prompt
    assert "3. 開発者コミュニティの動向" in prompt
    assert "4. 日本の開発者への示唆" in prompt
...
async def test_get_system_instruction(explorer):
    ...
    assert "V2EX" in instruction
    assert "開発者コミュニティ" in instruction
    assert "日本のエンジニア" in instruction
    assert "技術的な議論" in instruction


def test_v2ex_is_supported_platform_in_client():
    """v2exがTrendRadarでサポートされているかを確認."""
    assert "v2ex" in TrendRadarClient.SUPPORTED_PLATFORMS
...
async def test_collect_success(explorer, mock_trendradar_client):
    ...
    with patch.object(explorer, "_summarize_article", new_callable=AsyncMock) as mock_summarize:
        with patch.object(explorer, "_store_articles", new_callable=AsyncMock) as mock_store:
            ...
            assert len(result) == 1
            mock_trendradar_client.get_latest_news.assert_called_once_with(platform="v2ex", limit=5)
...
async def test_transform_to_article_valid(explorer):
    ...
    item = {
        "title": "Valid V2EX Title",
        "url": "http://example.com/valid",
        "desc": "Valid V2EX Description",
        "hot": "150",
    ...
    assert article.title == "Valid V2EX Title"
    assert article.url == "http://example.com/valid"
    assert article.text == "Valid V2EX Description"
