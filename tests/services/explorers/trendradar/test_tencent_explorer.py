...
def explorer(mock_trendradar_client):
    return TencentExplorer(storage_dir="test_data")


@pytest.mark.asyncio
async def test_initialization(explorer):
    """初期化のテスト."""
    assert explorer.service_name == "trendradar-tencent"
    assert explorer.PLATFORM_NAME == "tencent-hot"
    assert explorer.FEED_NAME == "tencent"
    assert explorer.MARKDOWN_HEADER == "腾讯新闻トレンド"
...
async def test_get_summary_prompt(explorer):
    ...
    assert "腾讯新闻ニューストレンド" in prompt
    assert "Test News Title" in prompt
    assert "Test News Description" in prompt
    assert "ニュースの概要" in prompt
    assert "重要なポイント" in prompt
    assert "3. 中国市場への影響" in prompt
    assert "4. 日本・国際社会との関連" in prompt
...
async def test_get_system_instruction(explorer):
    ...
    assert "腾讯新闻" in instruction
    assert "Tencent News" in instruction
    assert "日本のユーザー" in instruction
    assert "ニュースの要点" in instruction


def test_tencent_is_supported_platform_in_client():
    """tencent-hotがTrendRadarでサポートされているかを確認."""
    assert "tencent-hot" in TrendRadarClient.SUPPORTED_PLATFORMS
...
async def test_collect_success(explorer, mock_trendradar_client):
    ...
    with patch.object(explorer, "_summarize_article", new_callable=AsyncMock) as mock_summarize:
        with patch.object(explorer, "_store_articles", new_callable=AsyncMock) as mock_store:
            ...
            assert len(result) == 1
            mock_trendradar_client.get_latest_news.assert_called_once_with(platform="tencent-hot", limit=5)
...
async def test_transform_to_article_valid(explorer):
    ...
    assert article.title == "Valid News Title"
    assert article.url == "http://example.com/valid"
    assert article.text == "Valid News Description"
