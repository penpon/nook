...
def explorer(mock_trendradar_client):
    return SspaiExplorer(storage_dir="test_data")


@pytest.mark.asyncio
async def test_initialization(explorer):
    """初期化のテスト."""
    assert explorer.service_name == "trendradar-sspai"
    assert explorer.PLATFORM_NAME == "sspai"
    assert explorer.FEED_NAME == "sspai"
    assert explorer.MARKDOWN_HEADER == "少数派ホットトピック"
...
async def test_get_summary_prompt(explorer):
    ...
    assert "少数派（SSPai）ホットトピック" in prompt
    assert "Test Title" in prompt
    assert "Test Description" in prompt
    assert "コンテンツの概要" in prompt
    assert "3. 活用シーン" in prompt
    assert "4. 日本での代替・類似ツール" in prompt
...
async def test_get_system_instruction(explorer):
    ...
    assert "少数派" in instruction
    assert "SSPai" in instruction
    assert "日本のユーザーに向けて" in instruction
    assert "生産性向上" in instruction


def test_sspai_is_supported_platform_in_client():
    assert "sspai" in TrendRadarClient.SUPPORTED_PLATFORMS
...
async def test_collect_success(explorer, mock_trendradar_client):
    ...
    with patch.object(explorer, "_summarize_article", new_callable=AsyncMock) as mock_summarize:
        with patch.object(explorer, "_store_articles", new_callable=AsyncMock) as mock_store:
            ...
            assert len(result) == 1
            mock_trendradar_client.get_latest_news.assert_called_once_with(platform="sspai", limit=5)
