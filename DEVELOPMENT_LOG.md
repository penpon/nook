...
- REDDIT_USER_AGENTの値をダブルクォートで囲んで括弧によるエラーを解消
- 注意: 環境変数に特殊文字が含まれる場合は適切にクォートする必要がある

TASK-018: scripts/crawl_all.shのサービス名修正
- GitHub Trendingサービスの呼び出し時のサービス名を'github'から'github_trending'に修正
- 注意: run_servicesで使用するサービス名は正確に指定する必要がある
