...
def _create_empty_soup() -> BeautifulSoup:
    ...
    JuejinExplorerでは HTML コンテンツを使用しないため、プレースホルダーとして使用。
...
class JuejinExplorer(BaseService):
    ...
    # limit パラメータが指定されていない場合のデフォルト取得数 (5件)
    ...
    def __init__(self, storage_dir: str = "var/data", config: BaseConfig | None = None):
        ...
        super().__init__("trendradar-juejin", config=config)
    ...
    async def __aenter__(self) -> "JuejinExplorer":
        return self
    ...
    async def collect(
        ...
        *,
        ...
        news_items = await self.client.get_latest_news(
            platform="juejin", limit=effective_limit
        )
    ...
    async def _summarize_article(self, article: Article) -> None:
        ...
        prompt = f"""以下の掘金（Juejin）ホットトピックを日本語で詳細に要約してください。
