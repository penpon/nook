...
class BusinessFeed(BaseFeedService):
    ...
    def _get_summary_system_instruction(self) -> str:
        ...
        return """
        あなたはビジネスニュースの要約を行うアシスタントです。
        与えられた記事を分析し、簡潔で情報量の多い要約を作成してください。
        ビジネスの文脈や影響を重視し、経済・企業・市場の動向を正確に要約してください。
        回答は必ず日本語で行ってください。
        """
    ...
    def _get_summary_prompt_template(self, article: Article) -> str:
        ...
        1. 記事の主な内容（1-2文）
        2. ビジネス上の重要なポイント（箇条書き3-5点）
        3. 市場や企業への影響
        """

    def _needs_japanese_check(self) -> bool:
        """日本語判定が必要（business_feed特有）。"""
        return True
