...
// Qiitaのフィード情報から適切なフィード名を抽出
function extractQiitaFeedName(feedInfo: string): string {
...
export function parseQiitaArticlesMarkdown(markdown: string): ContentItem[] {
	...
	const feedGroups = new Map<
		string,
		{ title: string; url: string; content: string; feedInfo: string; tags: string }[]
	>();
		...
		if (line.startsWith("### ") && line.includes("[") && line.includes("](")) {
			...
			if (titleMatch) {
				...
				for (let j = i + 1; j < lines.length; j++) {
					...
					} else if (nextLine.startsWith("**タグ**:")) {
						// タグ情報の行
				...
				const feedName = extractQiitaFeedName(feedInfo);
