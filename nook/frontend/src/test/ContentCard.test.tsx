...
describe("ContentCard記事番号表示テスト", () => {
	describe("理想UI状態の記事番号「1」〜「15」表示", () => {
		it("indexが0の時、記事番号「1」が表示されること", () => {
			render(<ContentCard item={mockContentItem} darkMode={false} index={0} />);
		...
		it("indexが1の時、記事番号「2」が表示されること", () => {
			render(<ContentCard item={mockContentItem} darkMode={false} index={1} />);
		...
		it("indexが14の時、記事番号「15」が表示されること", () => {
			render(
				<ContentCard item={mockContentItem} darkMode={false} index={14} />,
			);
	...
	describe("番号バッジのスタイル確認", () => {
		...
		it("ダークモードで正しいクラスが適用されること", () => {
			render(<ContentCard item={mockContentItem} darkMode={true} index={0} />);
