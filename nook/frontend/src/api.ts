...
import { ContentResponse, WeatherResponse, ApiErrorResponse, ApiErrorContext, NetworkError, ServerError } from './types';
...
const isNetworkError = (error: unknown): error is NetworkError => {
  return typeof error === 'object' && error !== null && (error as Record<string, unknown>).isNetworkError === true;
};

const isServerError = (error: unknown): error is ServerError => {
  return typeof error === 'object' && error !== null && (error as Record<string, unknown>).isServerError === true;
};
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (isNetworkError(error)) {
        const delayMs = RETRY_DELAY * Math.pow(2, attempt); // Exponential backoff
        if (isDevelopment) {
          console.warn(`🔄 Retrying API call (attempt ${attempt + 1}/${retries + 1}) in ${delayMs}ms`);
        }
        await delay(delayMs);
      } else {
        throw error;
