        ...
        with open(log_file, encoding="utf-8") as f:
            for line in f:
                ...
                if line:
                    ...
                    log_entry["timestamp_dt"] = datetime.fromisoformat(
...
def aggregate_by_service(logs: list[dict]) -> list[dict]:
    ...
    for log in logs:
        ...
        # 最後の呼び出し時刻を更新
        if service_stats[service]["lastCalled"] is None or log[
            "timestamp_dt"
        ] > datetime.fromisoformat(
            service_stats[service]["lastCalled"].replace("Z", "+00:00")
        ):
...
def calculate_summary(logs: list[dict]) -> dict:
    """サマリー情報を計算します。"""
    if not logs:
        return {"todayTokens": 0, "todayCost": 0.0, "monthCost": 0.0, "totalCalls": 0}
...
async def get_usage_by_service():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to aggregate by service: {e}"
        )
...
async def get_daily_usage(days: int = 30):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to aggregate daily usage: {e}"
        )
